...
class WorkbookOutputService:
    ...
    def _resolve_current_final(self, case_id: str) -> FinalValuationSnapshotRecord:
        try:
    ...
    def generate(
        ...
        *,
        ...
        # First establish the upstream authority. The payload itself is frozen
        # under one persistence transaction below, then this same final snapshot
        # is authoritatively re-resolved before any workbook I/O begins.
        final_snapshot = self._resolve_current_final(case_id)
        ...
        with self._uow.atomic():
            ...
            if case is None or case.archived_at is not None:
                raise WorkbookOutputPrerequisiteError(
                    "appraisal case is missing or archived"
                )
            ...
            if subject is None or subject.archived_at is not None:
                raise WorkbookOutputPrerequisiteError(
                    "current subject property is required"
                )
            ...
            noncompliant = [
                ...
                and item.valuation_basis
                in {"OFFICIAL_LAND_PRICE", "OTHER_MANUAL_BASIS"}
            ]
            ...
            subject_characteristics = (
                self._uow.property_characteristics.list_for_property(
                    subject.property_id
                )
            )
            values: dict[str, str | int | bool | None] = {
                ...
                "subject.noncompliant_unit_price": noncompliant[
                    0
                ...
                "subject.depth": _required_characteristic(
                    subject_characteristics, "depth"
                ),
                "subject.shape": _required_characteristic(
                    subject_characteristics, "shape"
                ),
            ...
            comparables = tuple(
                sorted(comparables, key=lambda item: item.comparable_order)
            )
            ...
            # Freeze primitive payload and profile identifiers before releasing
            # the BEGIN IMMEDIATE read/write exclusion boundary.
            frozen_values = dict(values)
            frozen_profile_id = case.template_profile_id
            frozen_profile_version = case.template_profile_version
            frozen_binding = WorkbookGenerationSourceBinding(
                case_id=case_id,
                final_valuation_snapshot_id=final_snapshot.id,
                final_valuation_semantic_sha256=final_snapshot.semantic_sha256,
            )

        # Re-resolve after payload freeze. Any source/decision/final drift that
        # occurred before the transaction or immediately after it invalidates
        # the binding before workbook I/O is allowed to begin.
        revalidated = self._resolve_current_final(case_id)
        if (
            revalidated.id != final_snapshot.id
            or revalidated.semantic_sha256 != final_snapshot.semantic_sha256
        ):
            raise WorkbookOutputPrerequisiteError(
                "final valuation changed while workbook payload was being frozen"
            )
