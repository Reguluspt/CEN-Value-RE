...
export default function TemplateEditor({ open, templateName, groupId, displayName, onClose, onSuccess }) {
  const { user } = useAuth();
  ...
  const fetchDetail = useCallback(async (name) => {
    setLoading(true);
    try {
      const res = await getTemplateDetail(name, groupId);
      setDetail(res.data);
    } catch (err) {
      console.error(err);
  ...
  const fetchHistory = useCallback(async (name) => {
    ...
    try {
      const res = await getTemplateHistory(name, groupId);
      setHistory(res.data || []);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  }, [open, templateName, fetchDetail, fetchHistory, form]);
  ...
  const handleUpload = async (file) => {
    try {
      ...
      await uploadTemplateVersion(templateName, formData, groupId);
    ...
    <Drawer
      title={
        <Space>
          ...
          <span>Chi tiết mẫu: {displayName || templateName}</span>
        </Space>
      }
      placement="right"
      width={650}
