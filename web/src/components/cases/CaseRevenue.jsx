...
import { Card, Row, Col, Statistic, Table, Select, Space, Spin, Modal, Button, Form, Input, Tag, message } from 'antd';
...
import { formatCurrency, formatMillions } from './currency';
...
export default function CaseRevenue() {
  ...
  useEffect(() => {
    // Reload report data whenever one of its local filters changes.
...
Trân trọng cảm ơn sự hợp tác của Quý khách.`;
  ...
  const isDailyChart = Boolean(filters.execution_month);
  const revenueRows = isDailyChart
    ? stats?.daily_revenue || []
    : stats?.monthly_revenue || [];
  const chartData = revenueRows.map(r => ({
    period: isDailyChart ? r.label : `T${r.month.split('/')[0]}`,
    'Dự kiến': r.projected,
    'Đã thu': r.paid,
    'Công nợ': r.unpaid
  }));
  ...
  const periodLabel = filters.execution_month
    ? `tháng ${filters.execution_month}`
    : `năm ${filters.year}`;
  const chartTitle = isDailyChart
    ? `Biểu đồ doanh thu theo ngày trong tháng ${filters.execution_month}`
    : `Biểu đồ doanh thu năm ${filters.year}`;
  ...
  return (
    <div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
        <Col xs={12} sm={6}>
          <Card style={{ borderRadius: 12, border: '1px solid #d8e7e5', background: 'linear-gradient(135deg, #effaf8 0%, #d7f0ed 100%)' }}>
            <Statistic
              title={`Doanh thu dự kiến ${periodLabel}`}
              value={formatMillions(stats?.year_projected)}
        ...
        <Col xs={12} sm={6}>
          <Card style={{ borderRadius: 12, border: '1px solid #d8e7e5', background: 'linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%)' }}>
            <Statistic
              title={`Đã thanh toán ${periodLabel}`}
              value={formatMillions(stats?.year_paid)}
        ...
        <Col xs={12} sm={6}>
          <Card style={{ borderRadius: 12, border: '1px solid #d8e7e5', background: 'linear-gradient(135deg, #fff5f5 0%, #ffe4e6 100%)' }}>
            <Statistic
              title={`Công nợ tồn đọng ${periodLabel}`}
              value={formatMillions(stats?.year_unpaid)}
        ...
        <Col xs={12} sm={6}>
          <Card style={{ borderRadius: 12, border: '1px solid #d8e7e5', background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)' }}>
            <Statistic
              title={`Số lượng hồ sơ ${periodLabel}`}
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
        <Col xs={24} lg={14}>
          <Card style={{ borderRadius: 12, border: '1px solid #d8e7e5', height: '100%' }} title={chartTitle}>
            <div style={{ width: '100%', height: 350 }}>
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="period" stroke="#64748b" fontSize={12} minTickGap={12} />
                  <YAxis tickFormatter={formatMillions} width={72} stroke="#64748b" fontSize={11} />
                  <Tooltip
                    formatter={(value, name) => [formatCurrency(value), name]}
                    labelFormatter={(label) => isDailyChart && label !== 'Chưa xác định' ? `Ngày ${label}` : label}
                  />
                  <Legend />
                  <Bar dataKey="Đã thu" stackId="a" fill="#047857" barSize={isDailyChart ? 24 : 35} />
                  <Bar dataKey="Công nợ" stackId="a" fill="#c2413d" barSize={isDailyChart ? 24 : 35} />
        ...
        <Col xs={24} lg={10}>
          <Card 
            ...
            title={`Phân tích cơ cấu công nợ ${periodLabel}`}
            extra={
              <Select 
