...
import { Card, Table, Tag, Space, Button, message, Modal, Drawer, Input, List, Form, Badge, Checkbox, Select, Tooltip, Divider } from 'antd';
import { 
  EditOutlined, 
  DeleteOutlined, 
  PlusOutlined, 
...
import { buildAppraisalMailNotes } from '../../utils/gcnDetails';
...
export default function CaseTable({ filterOptions, onFilterOptionsRefresh }) {
  ...
  const [phathanhContactModalOpen, setPhathanhContactModalOpen] = useState(false);
  const [savingPhathanhContact, setSavingPhathanhContact] = useState(false);
  const [phathanhContactForm] = Form.useForm();
  ...
  const [customPhathanhDeliveryDetails, setCustomPhathanhDeliveryDetails] = useState(null);
  ...
  const currentPhathanhDeliveryDetails = customPhathanhDeliveryDetails !== null ? customPhathanhDeliveryDetails : selectedPhathanhDeliveryDetails;
  ...
  const handleAddPhathanhContact = async () => {
    try {
      const values = await phathanhContactForm.validateFields();
      setSavingPhathanhContact(true);
      const res = await createDeliveryContact({
        short_name: values.short_name,
        full_details: values.full_details,
      });
      await fetchPhathanhDeliveryContacts();
      if (res.data?.id) {
      ...
      setPhathanhContactModalOpen(false);
      phathanhContactForm.resetFields();
      ...
      setSavingPhathanhContact(false);
    }
  };
  ...
  const handleOpenPhathanhDeliveryModal = (record) => {
    ...
    setCustomPhathanhDeliveryDetails(null);
    phathanhDeliveryForm.setFieldsValue({
...
message.success('Xuất hồ sơ Word thành công! Đang tải về...');
  ...
  const handleOpenAppraisalMail = async (record) => {
    setActiveCaseForAppraisalMail(record);
    setAppraisalMailOpen(true);
    let caseData = record;
    try {
      const response = await getCase(record.id);
      caseData = response.data || record;
      setActiveCaseForAppraisalMail(caseData);
    } catch (err) {
      console.error(err);
    }
    appraisalMailForm.setFieldsValue({
      ...
      notes: buildAppraisalMailNotes(caseData),
    });
  };
...
open={appraisalMailOpen}
      ...
      >
        ...
        <Form form={appraisalMailForm} layout="vertical">
          <Form.Item name="notes" label="Ghi chú gửi kèm yêu cầu định giá">
            <Input.TextArea rows={10} placeholder="Thông tin GCN trích xuất và ghi chú hồ sơ" />
          </Form.Item>
        ...
        <Form form={phathanhDeliveryForm} layout="vertical">
          ...
          >
            <Select
              ...
              onChange={() => setCustomPhathanhDeliveryDetails(null)}
              ...
              dropdownRender={(menu) => (
                <>
                  ...
                  <Button
                    ...
                    icon={<PlusOutlined />}
                    block
                    onClick={() => setPhathanhContactModalOpen(true)}
                  >
                    Thêm người nhận mới vào danh bạ
                  </Button>
                </>
              )}
              options={[
          ...
          <Form.Item
            label={(
              <Space size={4}>
                ...
                <SwapAddressButton
                  value={currentPhathanhDeliveryDetails}
                  mode="owner"
                  onSwap={(val) => setCustomPhathanhDeliveryDetails(val)}
                />
              </Space>
            )}
          >
            <Input.TextArea
              value={currentPhathanhDeliveryDetails}
              onChange={(e) => setCustomPhathanhDeliveryDetails(e.target.value)}
              rows={4}
      ...
      <Modal
        open={phathanhContactModalOpen}
        title="Thêm danh bạ chuyển phát mới"
        okText="Lưu danh bạ"
        cancelText="Hủy"
        confirmLoading={savingPhathanhContact}
        onOk={handleAddPhathanhContact}
        onCancel={() => {
          setPhathanhContactModalOpen(false);
          phathanhContactForm.resetFields();
        }}
        destroyOnClose
      >
        <Form form={phathanhContactForm} layout="vertical">
          <Form.Item
            name="short_name"
            label="Tên gợi nhớ"
