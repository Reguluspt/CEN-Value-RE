...
import { Button, List, Space, Card, Radio, Select, message, Empty, Spin } from 'antd';
import { 
  FileWordOutlined, 
  FilePdfOutlined, 
  FileZipOutlined, 
...
import { getTemplateGroups } from '../../api/templates';
...
export default function CaseDocuments({ caseData, onCaseRefresh }) {
  ...
  const [templateGroupId, setTemplateGroupId] = useState('organization_default');
  ...
  const selectedTemplateGroup = templateGroups.find((group) => group.id === templateGroupId);
  const supportsAdvanceContract = selectedTemplateGroup?.templates?.some(
    (item) => item.role === 'hop_dong_tam_ung'
  );
  ...
  useEffect(() => {
    if (caseId) queueMicrotask(fetchDocuments);
  }, [caseId, fetchDocuments]);

  useEffect(() => {
    if (!isOrganization) return;
    queueMicrotask(() => {
      getTemplateGroups('organization')
        .then((response) => {
          const available = response.data || [];
          setTemplateGroups(available);
          setTemplateGroupId((current) => (
            available.some((group) => group.id === current && group.status === 'complete')
              ? current
              : available.find((group) => group.status === 'complete')?.id
          ));
        })
        .catch(() => message.error('Không thể tải danh sách bộ mẫu tổ chức'));
    });
  }, [isOrganization, caseId]);

  const handleGenerate = async () => {
    setGenerating(true);
    try {
      message.loading({ content: 'Đang tạo bộ văn bản...', key: 'gen' });
      await generateDocuments(caseId, paymentMethod, isOrganization ? templateGroupId : undefined);
      ...
      <Card style={{ borderRadius: 10, border: '1px solid #e5e7eb' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          <Space size="middle" wrap>
            {isOrganization && (
              <div style={{ marginRight: 8 }}>
                ...
                <Radio.Group onChange={(e) => setPaymentMethod(e.target.value)} value={paymentMethod}>
                  ...
                  <Radio value="advance" disabled={!supportsAdvanceContract}>Có tạm ứng</Radio>
                </Radio.Group>
              </div>
            )}
            ...
            {isOrganization && (
              <div>
                <span style={{ marginRight: 8, fontWeight: 500 }}>Chọn bộ mẫu:</span>
                <Select
                  value={templateGroupId}
                  onChange={(groupId) => {
                    setTemplateGroupId(groupId);
                    const group = templateGroups.find((item) => item.id === groupId);
                    if (!group?.templates?.some((item) => item.role === 'hop_dong_tam_ung')) {
                      setPaymentMethod('standard');
                    }
                  }}
                  style={{ width: 250 }}
                  placeholder="Chọn bộ mẫu hồ sơ"
            ...
            <Button
              ...
              loading={generating}
              disabled={isOrganization && !templateGroupId}
