...
import { Drawer, Descriptions, Tag, Button, List, Space, Empty, Spin, message, Card } from 'antd';
...
import { normalizeGcnDetails } from '../../utils/gcnDetails';
...
export default function SoboDetailDrawer({ open, record, onClose }) {
  ...
  const gcnDetails = normalizeGcnDetails(record.gcn_details);
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 24 }}>
        <h3 style={{ margin: '0 0 12px', fontSize: '14px', fontWeight: 600, color: 'rgba(0, 0, 0, 0.88)' }}>
          📄 Thông tin GCN trích xuất
        </h3>
        {gcnDetails.length > 0 ? (
          <Space direction="vertical" size={12} style={{ width: '100%' }}>
            {gcnDetails.map((detail, index) => (
              <Card
                key={`${detail.source_file_id || detail.source_file_name || 'gcn'}-${detail.asset_index}-${index}`}
                size="small"
                title={`GCN ${index + 1}${detail.source_file_name ? ` - ${detail.source_file_name}` : ''}`}
              >
                <Descriptions bordered column={1} size="small">
                  <Descriptions.Item label="Số thửa / Số tờ">
                    {detail.so_thua_dat ? `Thửa ${detail.so_thua_dat}` : '-'}
                    {detail.so_to_ban_do ? `, Tờ ${detail.so_to_ban_do}` : ''}
                  </Descriptions.Item>
                  <Descriptions.Item label="Địa chỉ thửa đất">
                    {detail.dia_chi_thua_dat || '-'}
                  </Descriptions.Item>
                  <Descriptions.Item label="Số GCN">
                    {detail.so_giay_chung_nhan || '-'}
                  </Descriptions.Item>
                  <Descriptions.Item label="Số vào sổ / Ngày cấp">
                    {detail.so_vao_so_cap_giay_chung_nhan || '-'}
                    {detail.ngay_cap_giay_chung_nhan ? ` / ${detail.ngay_cap_giay_chung_nhan}` : ''}
                  </Descriptions.Item>
                  <Descriptions.Item label="Chủ sử dụng / CCCD">
                    {detail.owner_name || '-'}
                    {detail.owner_citizen_id ? ` / ${detail.owner_citizen_id}` : ''}
                  </Descriptions.Item>
                  <Descriptions.Item label="Địa chỉ chủ sử dụng">
                    {detail.owner_address || '-'}
                  </Descriptions.Item>
                </Descriptions>
              </Card>
            ))}
          ...
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Chưa có thông tin GCN trích xuất" />
        )}
      </div>
