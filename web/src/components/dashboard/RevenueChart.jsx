...
export default function RevenueChart({ data = [], granularity = 'month', selectedMonth = '' }) {
  const isDaily = granularity === 'day';
  const xDataKey = isDaily ? 'label' : 'month';
  ...
  return (
    <Card style={{ borderRadius: 12, border: '1px solid #d8e7e5', height: '100%' }}>
      <Title level={4} style={{ fontSize: 16, fontWeight: 700, marginBottom: 20 }}>
        {isDaily
          ? `Doanh thu vs Công nợ theo ngày (${selectedMonth})`
          ...
          >
            ...
            <Tooltip 
              ...
              labelFormatter={(label) => {
                if (!isDaily) return `Tháng ${label}`;
                return label === 'Chưa xác định' ? label : `Ngày ${label}`;
              }}
