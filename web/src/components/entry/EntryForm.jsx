...
import { normalizeShortAssetDescription } from './assetDescription';
...
export default function EntryForm({ uploadId, formValues, onSaveSuccess }) {
  ...
  const formatPayload = (values) => {
    ...
    return {
      ...
      asset_description: normalizeShortAssetDescription(values.asset_description),
                ...
                <Form.Item
                  ...
                  label={(
                    <Space size={4}>
                      ...
                      <Form.Item noStyle shouldUpdate={(prev, curr) => prev.customer_address !== curr.customer_address}>
                        {({ getFieldValue, setFieldValue }) => (
                          <SwapAddressButton
                            value={getFieldValue('customer_address')}
                            mode="owner"
                            onSwap={(val) => setFieldValue('customer_address', val)}
                          />
                        )}
                      </Form.Item>
                    </Space>
          ...
          <Row gutter={16}>
            <Col span={24}>
              <Form.Item
                ...
                label={(
                  <Space size={4}>
                    <span>Tài sản thẩm định giá</span>
                    <Form.Item noStyle shouldUpdate={(prev, curr) => prev.asset_description !== curr.asset_description}>
                      {({ getFieldValue, setFieldValue }) => (
                        <SwapAddressButton
                          value={getFieldValue('asset_description')}
                          mode="annotate"
                          onSwap={(val) => setFieldValue('asset_description', val)}
                        />
                      )}
                    </Form.Item>
                  </Space>
              ...
              >
                <TextArea
                  ...
                  onBlur={(event) => {
                    const normalized = normalizeShortAssetDescription(event.target.value);
                    if (normalized !== event.target.value) {
                      form.setFieldValue('asset_description', normalized);
                    }
                  }}
                />
              </Form.Item>
        ...
        >
          ...
          <Form.Item
            ...
            label={(
              <Space size={4}>
                <span>Địa chỉ công ty</span>
                <Form.Item noStyle shouldUpdate={(prev, curr) => prev.address !== curr.address}>
                  {({ getFieldValue, setFieldValue }) => (
                    <SwapAddressButton
                      value={getFieldValue('address')}
                      mode="owner"
                      onSwap={(val) => setFieldValue('address', val)}
                    />
                  )}
                </Form.Item>
              </Space>
